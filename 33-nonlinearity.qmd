# Introduction to Non-linear Terms
...
### Raw quadratic fit after centering
...
### Overfitting and Limits on Predictor Counts
...
@Harrell2001 also mentions that if you have a **narrowly distributed** predictor, without a lot of variation to work with, then an even larger sample size $n$ should be required. See @Vittinghoff, Section 10.3 for more details.
...
- When we are attempting to **identify multiple independent predictors** (the explanatory model approach), then we will need to choose between collinear variables
    + options suggested by @Vittinghoff, p. 422, include choosing on the basis of plausibility as a causal factor, 
