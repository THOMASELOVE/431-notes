...
Note the use of the `ggstance` package here, which is new to us. We will also use the `ggpairs` function from the `GGally` package, and the `favstats` function from the `mosaic` package.
