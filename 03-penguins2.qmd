...
There are some other options for creating a graphical summary of a variable's distribution. For example, we might consider a density plot, as well as a rug plot running along the horizontal (X) axis:
...
Well, that's a problem. Looks like we have some missing values. Can we fix that, and also provide some additional summaries, like the sample size (*n*) and the median and standard deviation within each species? While we're at it, can we make it prettier?
