...
Note that though there are `r nrow(penguins)` rows in the tibble of data called `penguins`, only the first ten rows (penguins) are shown in the table above. Note also that the symbol `<NA>` is used to indicate a missing (not available) value.

## Create `newpenguins`: Eliminating Missing Data
