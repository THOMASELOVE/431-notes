...
To fix ideas, suppose we return to the Western Collaborative Group Study (WCGS) data we studied back in Chapter \@ref(WCGS-Study), which described 3154 men ages 39-59 who were free of heart disease and were employees of one of 10 California companies in 1960-1961. 

We are going to select a random sample of 500 of those men, with complete data on our outcome of interest, which is total cholesterol in mg/dl (`chol`) and on our three predictors of interest, which are systolic blood pressure in mm Hg (`sbp`), body mass index (`bmi`) and behavioral pattern (`behpat`), which is a four-category variable with levels A1, A2, B3 and B4. 
...
Next, we'll look at a scatterplot matrix, placing our outcome last, as usual.
...
```{r}
head(aug_chol1 %>% select(id:.resid)) %>% kable(dig = 3)
```
...
```{r}
head(aug_chol1 %>% select(id, .std.resid:.cooksd)) %>% kable(dig = 3)
```
...
```{r diag_wcgs_500_plot1, fig.height=5, fig.width=5, fig.align="center"}
...
```{r}
aug_chol1 %>% slice(113) %>% select(id:.std.resid) %>% kable(dig = 2)
```
...
```{r}
aug_chol1 %>% slice(113) %>%
    select(id:.std.resid) %>% kable(dig = 2)
```
...
Compare these findings to what we see in the `wcgs_500` data overall. Let's focus on the quantitative variables involved in our model, `sbp` and `bmi`.

```{r}
# requires library(mosaic)
...
ggplot(aug_chol1, aes(x = sbp, y = bmi)) +
    ...
    labs(x = "Systolic BP", y = "Body Mass Index",
         ...
         subtitle = "wcgs_500 data, model chol_1, 12 highest leverage points shown in red")
```
...
aug_chol1 %>% select(id, .std.resid, .hat, .cooksd) %>%
    arrange(desc(.cooksd)) %>% head() %>% kable(dig = 3)
```
