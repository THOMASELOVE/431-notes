...
Example 1 shows a function where the Pearson correlation is `r round_half_up(cor(correx3$x1, correx3$y1),3)` (a strong but not perfect linear relation), but the Spearman correlation is `r round_half_up(cor(correx3$x1, correx3$y1, method = "spearman"),2)` because the relationship is monotone, even though it is not perfectly linear. 
